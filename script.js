

let catalog = {
  "info": {
    "inventors": [
      {
        "logo" : "./assets/img/logo.png",
        "image": "./assets/img/joe.png",
        "miniName": "joe shuster",
        "role": "artist",
        "name": "Joe Shuster",
        "textInfo": "Joe Shuster war der Zeichner von Superman,den er 1938 zusammen mit Jerry Siegel erschuf. Geboren am 10. Juli 1914 in Toronto, prägte er das Aussehen des Superhelden. Trotz seines Erfolgs hatte Shuster finanzielle Schwierigkeiten und starb 1992. Sein Erbe lebt in Superman weiter.",
        "imageheart": "./assets/img/heartlike.png",
        "likes": 1250,
        "liked": false
      },
      {
        "logo" : "./assets/img/logo.png",
        "image": "./assets/img/jerry.png",
        "miniName": "jerry siegel",
        "role": "author",
        "name": "Jerry Siegel",
        "textInfo": "Jerry Siegel war der Autor von Superman, den er 1938 mit dem Zeichner Joe Shuster erschuf. Geboren am 17. Oktober 1914 in Cleveland, entwickelte Siegel die Idee des legendären Helden mit übermenschlichen Kräften. Auch Siegel lange Zeit mit finanziellen Problemen zu kämpfen. Er starb 1996 – doch seine kreative Vision lebt in Superman bis heute weiter.",
        "imageheart": "./assets/img/heartlike.png",
        "likes": 1110,
        "liked": false
      }
    ],
    
    "1930s": [
      {
        "image": "./assets/img/super1930.png",
        "title": "Action Comics #1",
        "year": 1938,
        "description": "Erster Auftritt von Superman",
        "creators": ["Jerry Siegel", "Joe Shuster"],
        "heartLike" : "./assets/img/heartlike.png",
        "likes": 1250,
        "liked": false,

        "comments": [
            {
              "name": "ComicFan01",
              "comment": "Einer der besten Superman-Stories aller Zeiten – absolut episch!"
            },
            {
              "name": "ClassicCritic",
              "comment": "Ein bisschen altmodisch erzählt, aber historisch sehr wichtig."
            },
            {
              "name": "PowerReader",
              "comment": "Interessante Wendung mit Supermans Kräften – habe ich so nicht erwartet!"
            },
            {
              "name": "ArchivKenner",
              "comment": "Ein echter Klassiker, gut gealtert. Ideal für Sammler."
            },
            {
              "name": "JustLooking",
              "comment": "Ich muss noch weiterlesen, aber das Cover hat mich sofort angesprochen."
            }
        ]
      },
      {
        "image": "./assets/img/super1940.png",
        "title": "Superman #1",
        "year": 1939,
        "description": "Erstes Solo-Heft",
        "creators": ["Jerry Siegel", "Joe Shuster"],
        "heartLike" : "./assets/img/heartlike.png",
        "likes": 1250,
        "liked": false,
        "comments": [
            {
              "name": "ComicFan01",
              "comment": "Einer der besten Superman-Stories aller Zeiten – absolut episch!"
            },
            {
              "name": "ClassicCritic",
              "comment": "Ein bisschen altmodisch erzählt, aber historisch sehr wichtig."
            },
            {
              "name": "PowerReader",
              "comment": "Interessante Wendung mit Supermans Kräften – habe ich so nicht erwartet!"
            },
            {
              "name": "ArchivKenner",
              "comment": "Ein echter Klassiker, gut gealtert. Ideal für Sammler."
            },
            {
              "name": "JustLooking",
              "comment": "Ich muss noch weiterlesen, aber das Cover hat mich sofort angesprochen."
            }
        ]
      },
      {
        "image": "./assets/img/super1950.png",
        "title": "Newspaper Strip",
        "year": 1939,
        "description": "Tägliche Comicstrips in Zeitungen",
        "creators": ["Jerry Siegel", "Joe Shuster"],
        "heartLike" : "./assets/img/heartlike.png",
        "likes": 1250,
        "liked": false,
        "comments": [
            {
              "name": "ComicFan01",
              "comment": "Einer der besten Superman-Stories aller Zeiten – absolut episch!"
            },
            {
              "name": "ClassicCritic",
              "comment": "Ein bisschen altmodisch erzählt, aber historisch sehr wichtig."
            },
            {
              "name": "PowerReader",
              "comment": "Interessante Wendung mit Supermans Kräften – habe ich so nicht erwartet!"
            },
            {
              "name": "ArchivKenner",
              "comment": "Ein echter Klassiker, gut gealtert. Ideal für Sammler."
            },
            {
              "name": "JustLooking",
              "comment": "Ich muss noch weiterlesen, aber das Cover hat mich sofort angesprochen."
            }
        ]
      }
    ],
  "1940s": [
  {
    "image": "./assets/img/super1960.png",
    "title": "Superman #17",
    "year": 1942,
    "description": "Superman kämpft erstmals gegen Lex Luthor.",
    "creators": ["Jerry Siegel", "Joe Shuster"],
    "heartLike": "./assets/img/heartlike.png",
    "likes": 870,
    "liked": false,
    "comments": [
        {
          "name": "GoldenAgeFan",
          "comment": "Lex Luthors erster Auftritt – legendär!"
        },
        {
          "name": "HistoryBuff",
          "comment": "Diese Comics zeigen Supermans frühe Heldenreise."
        },
        {
          "name": "SupermanForever",
          "comment": "Alte Klassiker, aber immer noch spannend zu lesen."
        }
    ]
  },
  {
    "image": "./assets/img/super1940.png",
    "title": "Action Comics #64",
    "year": 1943,
    "description": "Superman im Kampf gegen Spionage während des Zweiten Weltkriegs.",
    "creators": ["Jerry Siegel", "Leo Nowak"],
    "heartLike": "./assets/img/heartlike.png",
    "likes": 920,
    "liked": false,
    "comments": [
        {
          "name": "ComicArchivist",
          "comment": "Toller historischer Bezug – wichtig für Superman-Fans."
        },
        {
          "name": "VintageReader",
          "comment": "Starker Patriotismus in diesen Ausgaben."
        },
        {
          "name": "ActionHero",
          "comment": "Interessant, wie Superman gesellschaftliche Themen aufgreift."
        }
    ]
  },
  {
    "image": "./assets/img/super1940.png",
    "title": "Superman #38",
    "year": 1946,
    "description": "Superman entdeckt neue Kräfte und seine Beziehung zu Lois Lane entwickelt sich.",
    "creators": ["Jerry Siegel", "Wayne Boring"],
    "heartLike": "./assets/img/heartlike.png",
    "likes": 1030,
    "liked": false,
    "comments": [
        {
          "name": "LoisLaneFan",
          "comment": "Lois und Superman – einfach Kult!"
        },
        {
          "name": "GoldenComics",
          "comment": "Eine der besten Ausgaben nach dem Krieg."
        },
        {
          "name": "SupesHistory",
          "comment": "Superman entwickelt sich hier sehr menschlich weiter."
        }
    ]
  }
],

    "1950s": [
      {
        "image": "./assets/img/super1950.png",
        "title": "Superman's Pal Jimmy Olsen #1",
        "year": 1954,
        "description": "Start der Solo-Serie von Jimmy Olsen, Supermans Freund, mit kuriosen Abenteuern.",
        "creators": ["Otto Binder", "Curt Swan"],
        "heartLike" : "./assets/img/heartlike.png",
        "likes": 1120,
        "liked": false,
        "comments": [
            {
              "name": "ComicFan01",
              "comment": "Einer der besten Superman-Stories aller Zeiten – absolut episch!"
            },
            {
              "name": "ClassicCritic",
              "comment": "Ein bisschen altmodisch erzählt, aber historisch sehr wichtig."
            },
            {
              "name": "PowerReader",
              "comment": "Interessante Wendung mit Supermans Kräften – habe ich so nicht erwartet!"
            },
            {
              "name": "ArchivKenner",
              "comment": "Ein echter Klassiker, gut gealtert. Ideal für Sammler."
            },
            {
              "name": "JustLooking",
              "comment": "Ich muss noch weiterlesen, aber das Cover hat mich sofort angesprochen."
            }
        ]
      },
      {
        "image": "./assets/img/super1950.png",
        "title": "The Supergirl from Krypton!",
        "year": 1959,
        "description": "Erste Erscheinung von Supergirl (Kara Zor-El), Supermans Cousine.",
        "creators": ["Otto Binder", "Al Plastino"],
        "heartLike" : "./assets/img/heartlike.png",
        "likes": 1430,
        "liked": false,
        "comments": [
            {
              "name": "ComicFan01",
              "comment": "Einer der besten Superman-Stories aller Zeiten – absolut episch!"
            },
            {
              "name": "ClassicCritic",
              "comment": "Ein bisschen altmodisch erzählt, aber historisch sehr wichtig."
            },
            {
              "name": "PowerReader",
              "comment": "Interessante Wendung mit Supermans Kräften – habe ich so nicht erwartet!"
            },
            {
              "name": "ArchivKenner",
              "comment": "Ein echter Klassiker, gut gealtert. Ideal für Sammler."
            },
            {
              "name": "JustLooking",
              "comment": "Ich muss noch weiterlesen, aber das Cover hat mich sofort angesprochen."
            }
        ]
      },
      {
        "image": "./assets/img/super1950.png",
        "title": "The Battle with Bizarro!",
        "year": 1959,
        "description": "Debüt von Bizarro, einem fehlgeschlagenen Superman-Klon mit verzerrter Logik.",
        "creators": ["Otto Binder", "George Papp"],
        "heartLike" : "./assets/img/heartlike.png",
        "likes": 1260,
        "liked": false,
        "comments": [
            {
              "name": "ComicFan01",
              "comment": "Einer der besten Superman-Stories aller Zeiten – absolut episch!"
            },
            {
              "name": "ClassicCritic",
              "comment": "Ein bisschen altmodisch erzählt, aber historisch sehr wichtig."
            },
            {
              "name": "PowerReader",
              "comment": "Interessante Wendung mit Supermans Kräften – habe ich so nicht erwartet!"
            },
            {
              "name": "ArchivKenner",
              "comment": "Ein echter Klassiker, gut gealtert. Ideal für Sammler."
            },
            {
              "name": "JustLooking",
              "comment": "Ich muss noch weiterlesen, aber das Cover hat mich sofort angesprochen."
            }
        ]
      }
    ],
    "1960s": [
      {
        "image": "./assets/img/super1960.png",
        "title": "The Last Days of Superman!",
        "year": 1962,
        "description": "Superman glaubt, dass er sterben wird, und verabschiedet sich von seinen Freunden – ein emotionaler Klassiker.",
        "creators": ["Edmond Hamilton", "Curt Swan"],
        "heartLike" : "./assets/img/heartlike.png",
        "likes": 1600,
        "liked": false,
        "comments": [
            {
              "name": "ComicFan01",
              "comment": "Einer der besten Superman-Stories aller Zeiten – absolut episch!"
            },
            {
              "name": "ClassicCritic",
              "comment": "Ein bisschen altmodisch erzählt, aber historisch sehr wichtig."
            },
            {
              "name": "PowerReader",
              "comment": "Interessante Wendung mit Supermans Kräften – habe ich so nicht erwartet!"
            },
            {
              "name": "ArchivKenner",
              "comment": "Ein echter Klassiker, gut gealtert. Ideal für Sammler."
            },
            {
              "name": "JustLooking",
              "comment": "Ich muss noch weiterlesen, aber das Cover hat mich sofort angesprochen."
            }
        ]
      },
      {
        "image": "./assets/img/super1960.png",
        "title": "Superman's New Power!",
        "year": 1960,
        "description": "Superman entdeckt eine neue Energie-Projektil-Kraft, verliert aber vorübergehend seine anderen Fähigkeiten.",
        "creators": ["Jerry Siegel", "Wayne Boring"],
        "heartLike" : "./assets/img/heartlike.png",
        "likes": 1150,
        "liked": false,
        "comments": [
          
            {
              "name": "ComicFan01",
              "comment": "Einer der besten Superman-Stories aller Zeiten – absolut episch!"
            },
            {
              "name": "ClassicCritic",
              "comment": "Ein bisschen altmodisch erzählt, aber historisch sehr wichtig."
            },
            {
              "name": "PowerReader",
              "comment": "Interessante Wendung mit Supermans Kräften – habe ich so nicht erwartet!"
            },
            {
              "name": "ArchivKenner",
              "comment": "Ein echter Klassiker, gut gealtert. Ideal für Sammler."
            },
            {
              "name": "JustLooking",
              "comment": "Ich muss noch weiterlesen, aber das Cover hat mich sofort angesprochen."
            }

        ]
      },
      {
        "image": "./assets/img/super1960.png",
        "title": "The Super-Duel in Space",
        "year": 1961,
        "description": "Superman kämpft gegen Brainiac in einem intergalaktischen Showdown – ein Sci-Fi-Highlight der Ära.",
        "creators": ["Otto Binder", "Curt Swan"],
        "heartLike" : "./assets/img/heartlike.png",
        "likes": 1375,
        "liked": false,
        "comments": [
          
            {
              "name": "ComicFan01",
              "comment": "Einer der besten Superman-Stories aller Zeiten – absolut episch!"
            },
            {
              "name": "ClassicCritic",
              "comment": "Ein bisschen altmodisch erzählt, aber historisch sehr wichtig."
            },
            {
              "name": "PowerReader",
              "comment": "Interessante Wendung mit Supermans Kräften – habe ich so nicht erwartet!"
            },
            {
              "name": "ArchivKenner",
              "comment": "Ein echter Klassiker, gut gealtert. Ideal für Sammler."
            },
            {
              "name": "JustLooking",
              "comment": "Ich muss noch weiterlesen, aber das Cover hat mich sofort angesprochen."
            }
          ]
      }
    ],
  }
}

/* 
let img = document.getElementById("output");
img.src = catalog.info.inventors[0].image;  */// Setzt den src-Wert des img-Elements


/* console.log(catalog.info.inventors); */
